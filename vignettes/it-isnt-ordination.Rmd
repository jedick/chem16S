---
title: "It isn’t ordination: Plotting two chemical variables"
...
output:
  html_document:
    ...
    css: style.css
vignette: >
  %\VignetteIndexEntry{It isn’t ordination: Plotting two chemical variables}
...
The dataset is deposited in BioProject PRJNA860942, but the SRA data are not publicly available at the time of writing.
My predictions for this dataset:

* Positive correlation between `r Zc` of community reference proteomes and ORP.
* Because of input of reducing fluids, I expect to find lower `r Zc` than Humboldt Sulfuretum.
* If these samples have relatively low salinity, I expect to find higher `r nH2O` than Humboldt Sulfuretum.

```{r plot_metrics2.QinghaiTibet_T_ORP, fig.width = 6, fig.height = 5, fig.align = "center", pngquant = pngquant}
#psfile <- system.file("extdata/DADA2/ZFZ+23/ps_ZFZ+23.rds", package = "chem16S")
#ps <- readRDS(psfile)
#p2 <- plot_ps_metrics2(ps, color = "T") +
#  geom_point(size = 4) + labs(color = "T (°C)")
#p3 <- plot_ps_metrics2(ps, color = "ORP") +
#  geom_point(size = 4) + labs(color = "ORP (mV)")
#p2 / p3
```
