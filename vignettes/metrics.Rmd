...
Besides `r Zc` and `r nH2O`, chem16S can calculate elemental ratios (H/C, N/C, O/C, and S/C), grand average of hydropathicity (GRAVY), isoelectric point (pI), average molecular weight of amino acid residues, and protein length.

```{r other_metrics, fig.width = 8, fig.height = 5, fig.align = "center", pngquant = pngquant}
AAcomp <- taxon_AA[match(names(top10_classes), taxon_AA$organism), ]
...
plot.new()
legend("right", names(top10_classes), col = 1:10, pch = 1:10, bty = "n", xpd = NA)
```
...
In summary, chemical metrics provide insight into how environmental factors may shape the amino acid and elemental composition of proteins.
This is a new method in geochemical biology that can help to understand geochemical effects on genome evolution.
